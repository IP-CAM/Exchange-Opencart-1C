...
/*Приватні функції які виконують брудну роботу*/	
	...
	*/
	...
	private function updateProduct($product, $product_id = 0) {
		...
		// Зміним дату додавання для правильного відображення в модулі останніх
		$this->db->query("UPDATE " . DB_PREFIX . "product SET date_added = NOW() WHERE product_id = '" . (int)$product_id . "'");
	...
	public function checkDbSheme() {
		...
		$query = $this->db->query('SHOW TABLES LIKE "' . DB_PREFIX . 'product_option_description"');
		...
		$query = $this->db->query('SHOW TABLES LIKE "' . DB_PREFIX . 'product_option_value_description"');
		...
		// Создаем БД
			...
			$this->db->query(
					'CREATE TABLE 
						`' . DB_PREFIX . 'product_option_value_description` ( 
